...
user_collection = db.Table('user_collection', db.Column('gifs_id', db.Integer, db.ForeignKey('gifs.id')),db.Column('collection_id',db.Integer, db.ForeignKey('collections.id')))
...
def index():
    form = GifSearchForm()
    if form.validate_on_submit():
        search_term = get_or_create_search_term(db.session, form.search.data)
